...
class InputHelper(object):
    ...
    def loadW2V(self,emb_path, type="bin"):
        ...
        if type=="text":
            ...
            self.pre_emb =FastText.load_fasttext_format(emb_path)
            self.pre_emb.init_sims(replace=True)
            
            num_keys=len(self.pre_emb.wv.vocab)
    ...
    def getTsvDataCharBased(self, filepath):
        ...
        print("max_length of document sents is"+max_length)
        return np.asarray(x1),np.asarray(x2),np.asarray(y),max_length
    ...
    def getDataSets(self, training_paths, percent_dev, max_document_length,batch_size, is_char_based):
        if is_char_based:
